...
echo -e "\033[33m4. 使用默认地址\033[0m(\033[32mgithub难以连接时推荐选这个\033[0m) \033[33mhttps://mirror.ghproxy.com/https://github.com/avilliai/Manyana.git\033[0m"
...
echo -e "${RED}提示：如果可以直连GitHub，网速较快，建议直接使用以下地址：${NC}"
echo "wget https://github.com/avilliai/Manyana/releases/download/Manyana/ManyanaLauncher_V2_3.rar -O ManyanaLauncher_V2_3.rar"

echo "请选择下载地址："
echo -e "\033[33m1. 使用\033[0m(\033[32mgithub难以连接时推荐选这个\033[0m)\033[33m默认地址 https://mirror.ghproxy.com/https://github.com/avilliai/Manyana/releases/download/Manyana/ManyanaLauncher_V2_3.rar\033[0m"
echo -e "\033[33m2. 使用备用地址 https://mirror.ghproxy.com/https://github.com/avilliai/Manyana/releases/download/Manyana/ManyanaLauncher_V2_3.rar\033[0m"
echo -e "\033[33m3.\033[0m \033[31m直接使用GitHub地址\033[0m \033[32mhttps://github.com/avilliai/Manyana/releases/download/Manyana/ManyanaLauncher_V2_3.rar\033[0m"
...
# 根据用户选择设置下载地址
if [ "$choice" == "1" ]; then
    file_url="https://mirror.ghproxy.com/https://github.com/avilliai/Manyana/releases/download/Manyana/ManyanaLauncher_V2_3.rar"
elif [ "$choice" == "2" ]; then
    file_url="https://mirror.ghproxy.com/https://github.com/avilliai/Manyana/releases/download/Manyana/ManyanaLauncher_V2_3.rar"
elif [ "$choice" == "3" ]; then
    file_url="https://github.com/avilliai/Manyana/releases/download/Manyana/ManyanaLauncher_V2_3.rar"
else
    echo -e "\033[33m无效的选项，使用默认地址。\033[0m"
    file_url="https://mirror.ghproxy.com/https://github.com/avilliai/Manyana/releases/download/Manyana/ManyanaLauncher_V2_3.rar"
fi
...
if [ "$package_manager" = "apt" ];then
    sudo apt install -qq -y libgbm1 libasound2 libgtk-3-0 libnotify4 libxss1 libxtst6 xdg-utils libatspi2.0-0 libsecret-1-0 wget unrar screen git openjdk-17-jdk unzip
else
    sudo yum install -q -y epel-release wget unrar screen git java-17-openjdk java-17-openjdk-devel unzip xdg-utils mesa-libgbm alsa-lib gtk3 libnotify libXScrnSaver libXtst at-spi2-core libsecret
fi
...
if [ "$use_wget" == true ]; then
    wget https://mirror.ghproxy.com/https://github.com/avilliai/Manyana/archive/refs/heads/main.zip -O Manyana.zip
    if [ $? -eq 0 ]; then
        unzip Manyana.zip -d .
...
MAX_RETRIES=3
CHECK_INTERVAL=5
LOW_SPEED_THRESHOLD=102400  # 100 KB/s in bytes
LOW_SPEED_DURATION_LIMIT=30
...
download_file() {
    ...
    while [ $retries -lt $MAX_RETRIES ]; do
        wget -O ManyanaLauncher_V2_3.rar "$url" &
        local wget_pid=$!
        local start_time=$(date +%s)
        local low_speed_duration=0
        ...
        while kill -0 $wget_pid 2>/dev/null; do
            ...
            local file_size=$(stat -c%s ManyanaLauncher_V2_3.rar 2>/dev/null || echo 0)
            local speed=$((file_size / elapsed_time))

            if (( speed < LOW_SPEED_THRESHOLD )); then
                low_speed_duration=$((low_speed_duration + CHECK_INTERVAL))
            else
                low_speed_duration=0
            fi

            if (( low_speed_duration >= LOW_SPEED_DURATION_LIMIT )); then
                echo -e "\033[31m下载速度低于100KB/s超过30秒，重新下载...\033[0m"
                kill $wget_pid
...
echo -e "\033[32m正在下载ManyanaLauncher_V2_3.rar...\033[0m"

# 尝试下载文件
download_file "$file_url"
if [ $? -ne 0 ]; then
    echo -e "\033[31m下载失败，正在使用备用地址重试...\033[0m"
    download_file "https://mirror.ghproxy.com/https://github.com/avilliai/Manyana/releases/download/Manyana/ManyanaLauncher_V2_3.rar"
    if [ $? -ne 0 ]; then
...
# 解压ManyanaLauncher_V2_3.rar
echo -e "\033[33m正在解压ManyanaLauncher_V2_3.rar...\033[0m"
mkdir ManyanaLauncher
mv ManyanaLauncher_V2_3.rar ManyanaLauncher
unrar x ManyanaLauncher/ManyanaLauncher_V2_3.rar ManyanaLauncher
...
rm -f ManyanaLauncher/ManyanaLauncher_V2_3.rar
...
if [ "$system_arch" = "amd64" ]; then
    napcat_download_url="https://mirror.ghproxy.com/https://github.com/NapNeko/NapCatQQ/releases/download/v1.5.2/NapCat.linux.x64.zip"
    elif [ "$system_arch" = "arm64" ]; then
    napcat_download_url="https://mirror.ghproxy.com/https://github.com/NapNeko/NapCatQQ/releases/download/v1.5.2/NapCat.linux.arm64.zip"
else
